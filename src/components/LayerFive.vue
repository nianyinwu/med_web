            ...
            <tbody>
                <v-row no-gutters>
                    <v-col
                        ...
                        cols="4"
                    >
                     <div class="item" :style="getItemStyle(item)">
                        <div v-if="item.quantity==item.exact_quantity">{{ item.name }}</div>
                        <div v-if="item.quantity==item.exact_quantity">{{ item.quantity }} / {{ item.exact_quantity }}</div>
                        <div v-if="item.quantity!=item.exact_quantity">{{ item.name }}</div>
                        <div v-if="item.quantity!=item.exact_quantity">{{ item.quantity }} / {{ item.exact_quantity }}</div>
                        
                        <div class="red-text">{{ item.remark }}</div>
                            <v-btn
                            class="ma-0 pa-0"
                            text
                            @click="editItem(item)"
        ...
        methods: {
            ...
            getItemStyle(item) {
                if(item.name === 'O2 雙頭管'){
                    let height;
                    height='100px';
                    if (item.quantity == item.exact_quantity) {
                    return { backgroundColor: '#CEFFCE', height };
                    } else {
                    return { backgroundColor: '#FFD2D2', height };
                    }
                }
                if(item.name === '楊克式抽吸管'){
                    let height;
